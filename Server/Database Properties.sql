...
--NULL AS DataFileLocation,
--(SELECT physical_name FROM sys.master_files WHERE DB_NAME(database_id) = DB.name AND type_desc = 'rows') AS DataFileLocation,
(SELECT DISTINCT SUBSTRING(physical_name,1,3) AS physical_name FROM sys.master_files WHERE DB_NAME(database_id) = DB.name AND type_desc = 'rows') AS DataFileLocation,
(SELECT SUM((CONVERT(BIGINT, size)*8)/1024) FROM sys.master_files WHERE DB_NAME(database_id) = DB.name AND type_desc = 'rows') AS [DataFilesInMB],
(SELECT COUNT(1) FROM sys.master_files WHERE DB_NAME(database_id) = DB.name AND type_desc = 'log') AS LogFiles,
--NULL AS LogFileLocation,
--(SELECT physical_name FROM sys.master_files WHERE DB_NAME(database_id) = DB.name AND type_desc = 'rows') AS LogFileLocation,
(SELECT DISTINCT SUBSTRING(physical_name,1,3) AS physical_name FROM sys.master_files WHERE DB_NAME(database_id) = DB.name AND type_desc = 'rows') AS LogFileLocation,
(SELECT SUM((CONVERT(BIGINT, size)*8)/1024) FROM sys.master_files WHERE DB_NAME(database_id) = DB.name AND type_desc = 'log') AS [LogFilesInMB],
...
END) AS [CompatibilityLevelDescr],
...
is_cleanly_shutdown AS [CleanlyShutdown] 
